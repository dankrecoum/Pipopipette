...
class Planche:
    ...
    def valider_coup(self, index_ligne):
        ...
        TODO: Vous devez compléter le corps de cette fonction.
        """
        if index_ligne[2] not in "HV":
            return False, Exception("Votre orientation n'est pas valide, elle doit être H ou V")
        elif not self.coup_dans_les_limites(index_ligne):
            return False, Exception("Votre coup est hors des limites de la planche")
        elif self.lignes[index_ligne].jouee:
            return False, Exception("Ce coup a déjà été joué")
        else:
            return True, None
