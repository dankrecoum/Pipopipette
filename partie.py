...
class PartiePipopipette:
    ...
    def creer_joueur(self, couleur):
        ...
        TODO: Vous devez compléter le corps de cette fonction.
        """
        type_de_joueur = input(
            f"Quel type de joueur voulez-vous attribuer a la couleur {couleur} (Humain ou Ordinateur) : ")
        acceptable = type_de_joueur in ["Humain", "Ordinateur"]
    ...
    def jouer_tour(self):
        ...
        while not valide:
            print(f"{self.planche.valider_coup(coup_choisi)[1]} \n")  # affiche le message d'erreur approprié (
            # element en position 1 dans le tuple retourné )
            coup_choisi = self.joueur_courant.choisir_coup(self.planche)  # on choisit à nouveau
            valide = self.planche.valider_coup(coup_choisi)[0]
